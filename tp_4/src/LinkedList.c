...
{
    int returnAux =-1;
    void* Aux;//Auxiliar para no perder la referencia
...
/*
 *
 * if(order == 0)//Orden ascendiente
		  {
		    if(pFunc(Aux_2,Aux_3)==1) // retorno de la funcion que comprueba su orden
		    {

			 * Si pFunc retorna 1, Aux_2 es mayor que Aux_3

		      Aux = Aux_2;//Igualo al auxiliar para no perder la referencia
		      ll_set(this,i,Aux_3);
		      ll_set(this,j,Aux);
		      returnAux = 0;
		  }
		    ...
		    if(pFunc(Aux_2,Aux_3)==0)
		    {
|
		       * Si retorna 0, Aux_ 3 es mayor
